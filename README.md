...
Реализована матричная факторизация на PyTorch: латентные векторы юзеров и фильмов
(эмбеддинги) + bias-члены + global mean. Обучение через градиентный спуск (Adam),
L2-регуляризация (weight decay), early stopping по валидации.

### Методология
- **Split:** train / val / test = 70 / 10 / 20 по времени (temporal). Validation -
  для подбора гиперпараметров и early stopping; test использован один раз.
- **Early stopping** по val определяет число эпох (оптимум ~эпоха 43).
- **Подбор k** по валидации: оптимум **k = 5**. Больший k ведёт к переобучению -
  малый датасет не требует высокой латентной размерности.
- **Воспроизводимость:** зафиксирован random seed. Без него оптимальный k менялся
  от запуска к запуску, т.к. разница между близкими k была в пределах шума
  инициализации весов.

### Результаты (RMSE)

| Модель | Val RMSE | Test RMSE (весь test) |
|--------|----------|----------------------|
| Global mean (baseline) | - | 1.1191 |
| Item mean (baseline) | - | 1.0367 |
| MF (k=5) | 0.9846 | 1.1111 |

### Ключевые наблюдения
- **На покрытых парах** (юзер и фильм есть в train) MF даёт Val RMSE 0.9846 и
  **обходит** лучший baseline item-mean (1.0367): где у юзера есть история,
  латентные факторы работают лучше константы.
- **На всём тесте** MF (1.1111) уступает item-mean. Причина - cold start: при
  temporal split ~80% тестовых пар это новые юзеры/фильмы, для которых MF не имеет
  обученных векторов и откатывается на global_mean.
- Test RMSE (1.1111) > Val RMSE (0.9846) - корректное соотношение для честной
  оценки на невиданных данных.
- Смена k почти не влияет на общий test RMSE (~1.11), потому что ~80% предсказаний -
  это откат на global_mean, не зависящий от модели. Улучшение модели «тонет»
  в массе cold-start откатов.

### Вывод
MF **решает проблему sparsity** - выучивает плотные векторы и работает даже для
малоактивных, но известных юзеров, побеждая baseline там, где применима. Но MF
**не решает cold start**: для полностью новых юзеров/фильмов у неё нет векторов.
